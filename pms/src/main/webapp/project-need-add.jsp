...
<script type="text/javascript">
	$(function () {
		$.ajax({
			url:"${pageContext.request.contextPath}/pro/proListWithoutAnalyze",
			type:"get",
			dataType:"json",
			success:function (obj) {
				$(obj).each(function (index,item) {
					$("#id").append("<option value='"+item.pid+"'>"+item.pname+"</option>");
				})
			}
		});
	});
	...
	function save() {
		$.ajax({
			type:"post",
			url:"${pageContext.request.contextPath}/analyze/insertProAna",
			data:$("#form2").serialize(),//表单序列化，要求表单的name和实体类相同
			success:function (obj) {
				if(obj.status==200){
					alert(obj.msg);
